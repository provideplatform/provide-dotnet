...
namespace provide
{
    public class Vault: ApiClient
    {
        ...
        public async Task<(int, object)>CreateVault(string token, Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>ListVaults(string token, Dictionary<string, object> args) {
            var uri = String.Format("vaults");
        ...
        public async Task<(int, object)>ListVaultKeys(string token, string vaultID, Dictionary<string, object> args) {
            var uri = String.Format("vaults/{}", vaultID);
            return await this.Get(uri, args);
        }

        // CreateVault creates a new vault available within the key management service
        public async Task<(int, object)>CreateVaultKey(string token, string vaultID, Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>DeleteVaultKey(string token, string vaultID, string keyID) {
            var uri = String.Format("vaults/{}/keys/{}", vaultID, keyID);
            return await this.Delete(uri);
        }

        // SignMessage securely signs the given message
        public async Task<(int, object)>SignMessage(string token, string vaultID, string keyID, string msg) {
        ...
        public async Task<(int, object)>VerifySignature(string token, string vaultID, string keyID, string msg, string sig) {
