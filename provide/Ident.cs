...
namespace provide
{
    public class Ident: ApiClient
    {
        ...
        public async Task<(int, object)>CreateApplication(Dictionary<string, object> args) {
            return await this.Post("applications", args);
        }

        // UpdateApplication using the given API token, application id and args
        public async Task<(int, object)>UpdateApplication(string applicationID, Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>ListApplications(Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>GetApplicationDetails(string applicationID, Dictionary<string, object> args) {
            var uri = String.Format("applications/{0}", applicationID);
            return await this.Get(uri, args);
        }

        // ListApplicationTokens retrieves a paginated list of application API tokens
        public async Task<(int, object)>ListApplicationTokens(string applicationID, Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>CreateApplicationToken(string applicationID, Dictionary<string, object> args) {
            args["application_id"] = applicationID;
            return await this.Post("tokens", args);
        }

        // CreateToken creates a new API token.
        public async Task<(int, object)>CreateToken(Dictionary<string, object> args) {
            return await this.Post("tokens", args);
        }

        // ListTokens retrieves a paginated list of API tokens scoped to the given API token
        public async Task<(int, object)>ListTokens(Dictionary<string, object> args) {
            return await this.Get("tokens", args);
        }

        // GetTokenDetails retrieves details for the given API token id
        public async Task<(int, object)>GetTokenDetails(string tokenID, Dictionary<string, object> args) {
        ...
        public async Task<(int, object)>DeleteToken(string tokenID) {
            var uri = String.Format("tokens/{0}", tokenID);
        ...
        public async Task<(int, object)>CreateUser(Dictionary<string, object> args) {
            return await this.Post("users", args);
        }

        // ListUsers retrieves a paginated list of users scoped to the given API token
        public async Task<(int, object)>ListUsers(Dictionary<string, object> args) {
            return await this.Get("users", args);
        }

        // GetUserDetails retrieves details for the given user id
        public async Task<(int, object)>GetUserDetails(string userID, Dictionary<string, object> args) {
            var uri = String.Format("users/{0}", userID);
            return await this.Get(uri, args);
        }

        // UpdateUser updates an existing user
        public async Task<(int, object)>UpdateUser(string userID, Dictionary<string, object> args) {
            var uri = String.Format("users/{0}", userID);
            return await this.Put(uri, args);
        }
    }
